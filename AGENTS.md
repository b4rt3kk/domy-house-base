...
- For an explicitly approved multi-branch publication, call `publish_release_flow` once with the exact source branch, exact source HEAD SHA, and ordered target branches. Treat its per-stage verified SHA results as final. Do not repeat PR, branch, commit, or merge verification with other GitHub tools unless the result is inconsistent or the user explicitly requests independent verification.
